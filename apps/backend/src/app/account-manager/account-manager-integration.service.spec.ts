...
describe('AccountManagerIntegrationService', () => {
  ...
  let tokens: jest.Mocked<
    Pick<KeycloakM2mTokenService, 'getClientCredentialsToken'>
  >;
  ...
  function createService(): AccountManagerIntegrationService {
    return new AccountManagerIntegrationService(
      tokens as unknown as KeycloakM2mTokenService,
    );
  }
  ...
  it('deduplicates enrollment lookup input and batches Account Manager requests', async () => {
    ...
    await expect(
      createService().lookupPeopleByEnrollmentNumbers(input),
    ).resolves.toEqual([
      {
        userId: 'user-1',
        enrollmentNumber: '20240000',
    ...
    expect(tokens.getClientCredentialsToken).toHaveBeenCalledWith({
      audience: 'account-api',
      scope: 'users',
    });
    expect(mockedAxios.post).toHaveBeenCalledTimes(2);
    expect(mockedAxios.post.mock.calls[0][0]).toBe(
      'https://account.example/api/v1/users/enrollment-lookup',
    );
    expect(
      (mockedAxios.post.mock.calls[0][1] as { enrollmentNumbers: string[] })
        .enrollmentNumbers,
    ).toHaveLength(500);
    expect(
      (mockedAxios.post.mock.calls[1][1] as { enrollmentNumbers: string[] })
        .enrollmentNumbers,
    ).toEqual(['20249999']);
  ...
  it('looks up people by private identifiers without returning unmatched input', async () => {
    ...
    expect(mockedAxios.post.mock.calls[0][0]).toBe(
      'https://account.example/api/v1/users/identifier-lookup',
    );
  ...
  it('uses the default Account Manager origin and short-circuits empty lookups', async () => {
    ...
    await expect(
      createService().lookupPeopleByEnrollmentNumbers([' ', '']),
    ).resolves.toEqual([]);
    await expect(
      createService().lookupPeopleByIdentifiers([]),
    ...
    await expect(
      createService().lookupPeopleByEnrollmentNumbers(['20240001']),
    ).resolves.toEqual([]);
    expect(mockedAxios.post.mock.calls[0][0]).toBe(
      'https://account.cacic.com.br/api/v1/users/enrollment-lookup',
    );
  });
  ...
  it('rejects invalid lookup responses and wraps request failures', async () => {
    ...
    mockedAxios.post.mockResolvedValueOnce({
      data: { users: [{ requestId: 'member-1', name: ' ' }] },
    });
    await expect(
      createService().lookupPeopleByIdentifiers([
        {
