...
export class PollsService {
  ...
  async createPoll(input: SavePollDto, user: AuthenticatedPrincipal): Promise<Poll> {
    ...
    const directLink = this.resolvePollDirectLink(input);
  ...
  async updatePoll(id: string, input: SavePollDto, user: AuthenticatedPrincipal): Promise<Poll> {
    ...
    const directLink = this.resolvePollDirectLink(input, existing);
    const now = new Date();
  ...
  async submitResponse(id: string, input: SubmitPollResponseDto, user?: AuthenticatedPrincipal): Promise<PollResponse> {
    ...
    await this.publishPollResultsForResponse(poll.id, response);
  ...
  private async resolvePollMetadata(input: SavePollDto, existing?: PollMetadataData): Promise<PollMetadataData> {
    const votingStyle = this.toDbVotingStyle(input.votingStyle ?? 'secret');
    const voterEligibilitySource = this.toDbVoterEligibilitySource(
      input.voterEligibilitySource ?? 'authenticatedUsers',
    );
    const requireVerifiedUnespRole =
      input.requireVerifiedUnespRole === true && this.isComputerScienceEligibilitySource(voterEligibilitySource);
    const linkedEventId = this.cleanOptionalText(input.linkedEventId) ?? null;
  ...
  private resolvePollDirectLink(input: SavePollDto, existing?: PollDirectLinkData): PollDirectLinkData {
